...
public class SubscriptionServiceTests
{
    private static SubscriptionService CreateService(out InMemoryDataStore store, out IResend resend, out ITwilioSmsSender sms,
        out IBackgroundJobClient jobs, IDateTimeProvider? provider = null)
    {
    ...
    {
        var service = CreateService(out var store, out var resend, out _, out var jobs);
        ...
        var subscriber = (await store.GetEmailSubscribersAsync()).Single();
    ...
    {
        var service = CreateService(out var store, out var resend, out _, out _);
    ...
    {
        var service = CreateService(out var store, out var resend, out var sms, out var jobs);
        ...
        await resend.Received().EmailSendAsync(Arg.Any<EmailMessage>(), Arg.Any<CancellationToken>());
        var subscriber = (await store.GetSmsSubscribersAsync()).Single();
        Assert.False(subscriber.IsVerified);
    }
    ...
    public async Task CountExpiredUnverifiedAsync_counts_old_records()
    {
        ...
        var service = CreateService(out var store, out _, out _, out _, provider);
