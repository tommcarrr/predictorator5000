...
public class InMemoryDataStore : IDataStore
{
    ...
    {
        subscriber.Id = _nextEmailId++;
        EmailSubscribers.Add(subscriber);
        return Task.CompletedTask;
    }

    public Task<Subscriber?> GetEmailSubscriberByVerificationTokenAsync(string token) =>
        Task.FromResult<Subscriber?>(EmailSubscribers.FirstOrDefault(s => s.VerificationToken == token));

    public Task<Subscriber?> GetEmailSubscriberByUnsubscribeTokenAsync(string token) =>
        Task.FromResult<Subscriber?>(EmailSubscribers.FirstOrDefault(s => s.UnsubscribeToken == token));
    ...
    public Task<Subscriber?> GetEmailSubscriberByEmailAsync(string normalizedEmail) =>
        Task.FromResult<Subscriber?>(EmailSubscribers.FirstOrDefault(s => s.Email.ToLower() == normalizedEmail));
    ...
    public Task<Subscriber?> GetEmailSubscriberByIdAsync(int id) =>
        Task.FromResult<Subscriber?>(EmailSubscribers.FirstOrDefault(s => s.Id == id));

    public Task UpdateEmailSubscriberAsync(Subscriber subscriber)
    {
        var existing = EmailSubscribers.FirstOrDefault(s => s.Id == subscriber.Id);
        if (existing != null)
        {
    ...
    {
        subscriber.Id = _nextSmsId++;
        SmsSubscribers.Add(subscriber);
        return Task.CompletedTask;
    }

    public Task<SmsSubscriber?> GetSmsSubscriberByVerificationTokenAsync(string token) =>
        Task.FromResult<SmsSubscriber?>(SmsSubscribers.FirstOrDefault(s => s.VerificationToken == token));

    public Task<SmsSubscriber?> GetSmsSubscriberByUnsubscribeTokenAsync(string token) =>
        Task.FromResult<SmsSubscriber?>(SmsSubscribers.FirstOrDefault(s => s.UnsubscribeToken == token));
    ...
    public Task<SmsSubscriber?> GetSmsSubscriberByIdAsync(int id) =>
        Task.FromResult<SmsSubscriber?>(SmsSubscribers.FirstOrDefault(s => s.Id == id));

    public Task UpdateSmsSubscriberAsync(SmsSubscriber subscriber)
    {
        var existing = SmsSubscribers.FirstOrDefault(s => s.Id == subscriber.Id);
        if (existing != null)
        {
