...
public class InMemoryGameWeekRepository : IGameWeekRepository
{
    ...
    public Task<GameWeek?> GetGameWeekAsync(string season, int number) =>
        Task.FromResult<GameWeek?>(Items.FirstOrDefault(g => g.Season == season && g.Number == number));

    public Task<GameWeek?> GetByIdAsync(int id) =>
        Task.FromResult<GameWeek?>(Items.FirstOrDefault(g => g.Id == id));

    public Task<GameWeek?> GetNextGameWeekAsync(DateTime date)
    {
        var result = Items.Where(g => g.EndDate >= date)
    ...
    {
        var existing = Items.FirstOrDefault(g => g.Id == id);
        if (existing != null)
